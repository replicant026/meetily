...
Adapter de serviço — abstrai comunicação com backends de transcrição. Ponte entre React (Steps 6-7, 9, 11) e Rust. Exporta funções que o frontend chama; internamente invoca Tauri commands. Step 2 (servidor legado de transcrição) é a origem histórica dos dados que este módulo normaliza. Âncora da camada Service — mais importado que qualquer outro módulo em `frontend/src/lib/`.
