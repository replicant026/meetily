...
export function RecordingStateProvider({ children }: { children: React.ReactNode }) {
  ...
  // Track current status in a ref so setStatus keeps a stable identity
  ...
  const setStatus = useCallback((status: RecordingStatus, message?: string) => {
    logger.log(`[RecordingState] Status: ${statusRef.current} → ${status}`, message || '');
  ...
  const syncWithBackend = async () => {
    try {
      ...
      logger.log('[RecordingStateContext] Synced with backend:', backendState);
    } catch (error) {
  ...
  const startPolling = () => {
    ...
    logger.log('[RecordingStateContext] Starting state polling (500ms interval)');
  ...
  const stopPolling = () => {
    if (pollingIntervalRef.current) {
      logger.log('[RecordingStateContext] Stopping state polling');
      clearInterval(pollingIntervalRef.current);
   ...
   */
  useEffect(() => {
    logger.log('[RecordingStateContext] Setting up event listeners');
    ...
    const setupListeners = async () => {
      try {
        ...
        const unlistenStarted = await recordingService.onRecordingStarted(() => {
          logger.log('[RecordingStateContext] Recording started event');
          setState(prev => ({
            ...prev,
            isRecording: true,
            isPaused: false,
        ...
        const unlistenStopped = await recordingService.onRecordingStopped((payload) => {
          logger.log('[RecordingStateContext] Recording stopped event:', payload);
          setState(prev => {
        ...
        const unlistenPaused = await recordingService.onRecordingPaused(() => {
          logger.log('[RecordingStateContext] Recording paused event');
          setState(prev => ({
            ...prev,
            isPaused: true,
            isActive: false,
        ...
        const unlistenResumed = await recordingService.onRecordingResumed(() => {
          logger.log('[RecordingStateContext] Recording resumed event');
          setState(prev => ({
            ...prev,
            isPaused: false,
            isActive: true,
        ...
        logger.log('[RecordingStateContext] Event listeners set up successfully');
      } catch (error) {
    ...
    return () => {
      logger.log('[RecordingStateContext] Cleaning up event listeners');
   ...
   */
  useEffect(() => {
    logger.log('[RecordingStateContext] Initial mount - syncing with backend');
    syncWithBackend();
  }, []);
