...
/// Pure function: map a recording state string to a tray label.
pub fn tray_label_for_state(state: &str) -> String {
    match state {
        "recording" => "Meetily — Recording".to_string(),
        "processing" | "stopping" | "saving" => "Meetily — Processing meeting".to_string(),
        "error" => "Meetily — Attention needed".to_string(),
        _ => "Meetily".to_string(),
    }
}

/// Pure function: determine available tray actions for a given state.
pub fn tray_actions_for_state(state: &str) -> Vec<TrayAction> {
    let mut actions = vec![TrayAction {
        id: "show_meetily".to_string(),
        label: "Show Meetily".to_string(),
    }];

    match state {
        "recording" => {
            actions.push(TrayAction {
